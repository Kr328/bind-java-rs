...
use bind_java::{bind_java, call, DefaultClassLoader};
...
fn main() {
    with_java_vm(|ctx| unsafe {
        let class_loader = DefaultClassLoader::default();
        ...
        let c_standard_charsets = JavaStandardCharsets::find_class(ctx, &class_loader).unwrap();
        ...
        let c_charset = JavaCharset::find_class(ctx, &class_loader).unwrap();
        ...
        let c_char_buffer = JavaCharBuffer::find_class(ctx, &class_loader).unwrap();
        ...
        let c_system = JavaSystem::find_class(ctx, &class_loader).unwrap();
        let b_system = JavaSystem::bind(ctx, c_system).unwrap();
        let o_out: jobject = b_system.get_out(ctx, c_system).unwrap();

        let c_print_stream = JavaPrintStream::find_class(ctx, &class_loader).unwrap();
